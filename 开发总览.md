...
├── lang/zh-CN.json
├── 策划文件/                    ← 规则裁决依据（第五节）
├── docs/                        ← 规则书单页版；同时是 GitHub Pages 站点根目录
│   ├── index.html               ← 与 策划文件/规则书.md 同源，自带样式、无外部依赖
│   └── .nojekyll
...
上面两份是现行的，其余全部移进了 `策划文件/归档/`：

| 归档文件 | 原内容 |
|---|---|
| `大纲.txt` | 系统整体规则大纲 |
| `角色卡设计.txt` / `角色卡设计文档.md` / `详细角色卡设计.txt` | 角色卡规格：颜色、布局尺寸、三大 Tab |
| `对抗.md` | 对抗完整流程：发起→聊天框→拼点→承受，含 UI 弹窗规格 |
| `物品卡设计.md` / `物品卡-装备.md` / `物品卡-技能.md` / `物品卡-材料消耗品.md` / `物品卡-容器.md` | 各类物品卡规格 |

**归档里的都是旧版本**——对应的东西已经做进系统了，留作历史参考，**不是规则依据**。
和代码对不上时以代码为准。背景系统没有独立策划文件，规格同样以代码为准。
...
- **星芒与经验常量**（`config.mjs`）：`SKILL_COSTS {1:1, 2:5, 3:10}`；`EGO_COSTS {ZAYIN:1, TET:3, HE:5, WAW:10, ALEPH:15}`；`LEVEL_XP` 覆盖 Lv1–50（`MAX_LEVEL`），读取一律走 `getXpForLevel()` 而不是直接索引表——直接索引会让 50 级之后的 ×1.5 递推失效。`LEARN_POINTS_PER_LEVEL = 1`：每升 1 级得 1 点学习点数，存在 `system.learnPoints` 上、**可跨级保存**；`TRAIN_UPGRADE_COST {1:1, 2:2, 3:5}`：强化一阶的花费按**技能自身等级**收，取值走 `trainUpgradeCost()`。
...
- **`severing`（截断骰）没做完**（保留，待实现）：`config.mjs` 的 `DICE_TYPES` 与 `lang/zh-CN.json`（「截断」）都已定义，`assets/icons/Base_icon/截除硬币.webp` 图标也有，但 `clash.mjs` 从未读取 `diceType === "severing"`，技能卡的骰子类型下拉也只列了「一般骰子」「不可摧毁」。要落地至少需要：① 技能卡下拉补选项 ② `clash.mjs` 拼点判定里补 severing 分支 ③ 演出层的碎币表现（参考 `unbreakable` 的 `is-broken` 留场处理）④ 规则细节补进 `策划文件/规则书.md`。
