...
> 规模参考：`module/` 下 28 个 `.mjs`，约 25,200 行；`templates/` 25 个 `.hbs`。
...
│   └── {gm-console,squad-hud,quick-action-hud,sin-resource-hud}.hbs, combat/combat-hud.hbs
└── module/                      ← 28 个 .mjs
...
- **`severing`（截断骰）是死选项**：`config.mjs` 与 `lang/zh-CN.json` 都有定义，`clash.mjs` 从未读取，技能卡下拉也没列出。要么接上，要么清掉。
- **`templates/combat/combat-hud.hbs`** 已在预加载列表里，但没有任何 Application 类使用它——遗留脚手架，可清理。
