...
| `speed` | `1D6 + 敏捷` |
| `ap` | **行动币，无上限**（技能/装备/消耗品可提升）。回合开始补满至 3，超过 3 的保留。使用技能与发起对抗都不消耗；只用于**拼点失败重投**：一般骰每次重投累计 -1 拼点威力（仅本次对抗、不影响伤害），不可摧毁骰免惩罚；对抗结束胜方摧毁败方 1 枚（不可摧毁骰除外）；本次对抗内每拼点 3 次（不论胜负平局，含首投）最终威力 +1，不封顶，对抗结束时统一结算给胜方 |
