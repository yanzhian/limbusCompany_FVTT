...
`value` 是**字符串**，支持骰子公式（`"1D6"`）与正负号；带 `=` 前缀表示赋值而非增减。

| type | 含义 |
|---|---|
| `hpAdj` / `sanityAdj` / `apAdj` | 生命 / 理智 / 行动值 |
| `diceAdj` | 骰数 |
| `diceFacesAdj` | 面数 |
| `baseValue` | 基础值 |
| `weightAdj` | 攻击容量 |
| `atkAdj` / `defAdj` | 攻击 / 防御等级 |
| `seismicBlast` | 震颤引爆，`value` 是次数 |
| `extraDamage` | 追加伤害，另有 `dmgCategory`、`dmgSinType` |
