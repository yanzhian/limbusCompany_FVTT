...
| `allyTag` | `target`（`bgTag`/`bgTagOther`/`allTeamOther`…）、`targetTag`、`targetTagCount`、`perEach`、`maxTimes` | 场上有没有符合条件的友方（「若有其他背景带有X的友方」）；`perEach: true` 时人数也当倍数。**`target: "allTeam"` 例外**：那是「**本次目标是不是友方**」（【二元性】技能用它写「若目标为友方」），不问场上有没有人 |
...
// 相关技能转换（换卡，形态切换用）
// relDuration: "afterUse"（换上来的形态被投出去一次后还原）/ "afterClash"（本次结算后还原）/
//              "endOfTurn"（本回合结束时还原）/ "untilRest"（长休时还原）/
//              "permanent"（字段缺省值，**长休也不还原**）
//
// 【写卡时怎么选】需求文本一般不写时长，按下表定，不要自己发挥：
//   需求没提时长，触发是 [回合开始时] 之类  → "afterUse"      ← 默认
//   需求里写「本回合内」                    → "endOfTurn"
//   触发时机是 [攻击前]                     → "afterClash"
//   需求里强调「永久」/「直到休息」          → "untilRest"
//   真的要一辈子留着（长休也不掉）           → "permanent"
// 注意字段的**缺省值**是 permanent，而**默认写法**是 afterUse —— 别把它留空。
//
// permanent 与 untilRest 的差别只有一条：permanent 不登记还原任务，所以长休
// 也不会把它变回去，只能靠另一条转换换回来。绝大多数「永久强化」其实是
// untilRest —— 跨战斗保留，但下次休整回到起点。
 ...
 *     注意这只清登记过的四种（afterUse / afterClash / endOfTurn / untilRest）——
 *     `permanent` 根本不登记还原任务，**长休也不会把它变回去**，
 *     要收回只能靠另一条转换把它换回来。
 *   - "untilRest" 没有自己的到期检查：它只是"登记了但没有更早的到期条件"，
 *     于是自然活到长休那次 "all" 清账。
 */

/* ⚠️ 转换**改变不了当前这次对抗**——它是给"下一次"发牌用的。
 *
 * 三层原因，任改一层都不成立：
 *   1. 本次拼点的骰值与胜负在 resolution 里，**activity 跑之前**就算完了，
 *      之后再改什么都不影响这次的数字；
 *   2. 转换换的是**技能槽里的卡**，而本次对抗握着的是原来那个 item 对象，
 *      连 [攻击前] 之后那次"公式变了就重投"的检查读的也还是老卡；
 *   3. [受到伤害时] 只在**输了拼点、正在挨打**时派发——反击技能输了就没有反击伤害，
 *      "在这个时机把反击换成更强的形态再打出去"这个流程本身不存在。
 *
 * 想表达「这一发变成 XXX」，不要换卡，改这张卡自己的数值：
 *   触发时机 [攻击前] 或 [拼点时]（这两个之后有重投检查，改动能真正生效）
 *   + 前置/消耗照写 + 效果用 baseValue / diceAdj / diceFacesAdj / diceTypeChg。
 *   这些是"本次攻击内有效"的临时改动，[攻击后] 系统自动还原，**不要**自己写还原。
 *
 * 想表达「下一发变成 XXX」，才用 relatedSkillConvert + afterUse。
 */
...
| `relDuration` 留空 | 缺省是**永久**；需求没写时长时的**默认写法是 `afterUse`**，必须显式填 |
| 把「永久强化」写成 `permanent` | 那是长休也不掉的；跨战斗保留、休息归零的用 `untilRest` |
| 想用转换改「这一发」的数值 | 转换影响不了当前对抗；本发要变就用 `baseValue`/`diceAdj`/… 挂 [攻击前] |
