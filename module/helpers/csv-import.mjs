...
export const COLUMN_ALIASES = {
  ...
  // 价格 = 物品的「眼」价：卡面显示与商人报价（merchant-sheet 的 baseCostOf）
  // 读的都是 system.cost。早先这里映射到 system.price——那是个全仓无人读取的
  // 遗留字段，已随本次改动从各 schema 删除，导入的价格等于写进了黑洞。
...
const TRAIN_LEVEL_ALIASES = {
  "Ⅰ": 1, "I": 1, "1": 1,
  // 基线是 Ⅱ：「基础」指卡面顶层那一套，也就是 Ⅱ 阶
  "Ⅱ": 2, "II": 2, "2": 2, "基础": 2, "默认": 2,
  "Ⅲ": 3, "III": 3, "3": 3, "熟练": 3,
  "Ⅳ": 4, "IV": 4, "4": 4, "精通": 4,
  "Ⅴ": 5, "V": 5, "5": 5, "强化": 5,
};
...
function parseTrainCell(text) {
  ...
  return { __error: `「${t}」既不是训练等级（Ⅰ～Ⅴ / 1～5 / 基础·熟练·精通·强化）也不是 E.G.O 形态（觉醒/侵蚀）` };
}
...
export function buildTemplateCSV(itemType) {
  ...
  // 列头会重复（配方表的「量」出现好几次），所以示例值支持 `量#2` 这种按出现次序
  // 定位的键；没写编号的键作为兜底
  const seen = new Map();
  const example = cols.map(c => {
    const n = (seen.get(c) ?? 0) + 1;
    seen.set(c, n);
    return esc(sample[`${c}#${n}`] ?? sample[c] ?? "");
  });
...
const COLUMN_NOTES = {
  ...
  "训练等级": "一列两用：基础/守备填 Ⅰ～Ⅴ（或 1～5；Ⅱ/Ⅲ/Ⅳ/Ⅴ 也可写作 基础/熟练/精通/强化，基线是 Ⅱ），E.G.O 填 觉醒/侵蚀。同名的几行会合并成同一张卡的多套数值，留空的格子沿用打底那一行——基础/守备沿用低一阶，侵蚀沿用觉醒。共用列（基础/守备：名称·类型·分类·罪孽·等级·标签；E.G.O：名称·罪孽·EGO等级·罪孽资源消耗·抗性修改·标签·骰子类型）一律取打底那一行的值",
