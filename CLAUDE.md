...
- **The 「完成」 column means *overwrite*, not *skip*** — a truthy value marks the row as authoritative, and the import updates the existing world/compendium item of the same name **and type** instead of creating a duplicate (no match → create). `stripActivities()` in `csv-import-dialog.mjs` keeps the payload off `system.activities` and its two variant copies, because Activities are authored in the effect editor and a CSV round-trip would wipe them. It used to skip such rows outright, which made the sheet a write-once artifact.
- **图片是按物品名自动匹配的**（`module/helpers/asset-lookup.mjs`）—— CSV 没填「图片」列时，导入按 Item 类型选目录（材料/消耗品/容器 → `assets/material`，装备 → `assets/equipment`，技能 → `assets/skill`，恐惧卡 → `assets/panice`，背景 → `assets/background`），用 `FilePicker.browse` 扫一遍建 名→路径 表，归一化（去空白与 `·-_` 等分隔符）后取同名图。手填的 img 优先；覆盖导入时只在原物品还是 Foundry 默认图标时才补。
...
- **训练等级 强化 is bought with 学习点数**:每升 1 级得 `LEARN_POINTS_PER_LEVEL` (1) 点, banked on the actor as `system.learnPoints` — **points carry over**, unspent points are never forfeited (this replaced an older "one free quota per 3 levels, use it or lose it" rule). Upgrading one stage costs by the **skill's own level**, not the target stage: `TRAIN_UPGRADE_COST {1:1, 2:2, 3:5}` via `LIMBUSCOMPANY.trainUpgradeCost(level)`. `applyTrainUpgrade()` is where the charge happens (not the dialog), so macros and any other entry point pay too. There are two UIs over it: step 2 of `LevelUpDialog`, and the standalone `LearnPointsDialog` (角色卡等级旁的【学】按钮, always visible) — the latter exists because points are a *balance*, not a per-level-up quota: a player who clicked past step 2 can come back, and points can also come from quests or NPC tutelage, which the GM grants by editing the number there (logged to chat). Candidates come from `getTrainUpgradeCandidates()`: **all** non-E.G.O skills the actor owns, with the 6 basic + 1 defense equipped ones sorted first and slot-labelled (a skill only in the list can be upgraded too); each row carries `cost` / `affordable`. A stage with no authored `trainForms.lvN` data is shown disabled, since spending points there would change nothing.
